...
#
def _apply_processing(nr, footprint, three_d, func=median_filter):
    if three_d:
        pancake = np.swapaxes(np.tile(footprint, (3, 1, 1)), 0, -1)
        nr = 1.0*func(nr, footprint=pancake)
    else:
        nt = nr.shape[2]
        for i in range(0, nt):
            nr[:, :, i] = 1.0*func(nr[:, :, i], footprint=footprint)
    return nr


def _save_mp4(develop, new, name):
    filename = develop['img'] + '_{:s}.mp4'.format(name)
    aware_utils.write_movie(new, filename)
    return filename


def _save_pkl(develop, new, name):
    filename = develop['dat'] + '_{:s}.pkl'.format(name)
    f = open(filename, 'wb')
    pickle.dump(new, f)
    f.close()
    return filename
...
def _save_npy(develop, new, name):
    filename = develop['dat'] + '_{:s}.npy'.format(name)
    f = open(filename, 'wb')
    np.save(f, new)
    f.close()
    return filename
...
def processing(mc, radii=[[11, 11]*u.degree],
    ...
    if develop:
        develop_filepaths['np_meta'] = _save_pkl(develop, mc.all_meta(), 'np_meta')
        develop_filepaths['np_nans'] = _save_pkl(develop, nans_here, 'np_nans')

    # Create the list that will be turned in to a mapcube
    for i, m in enumerate(new):
        new_map = Map(ma.masked_array(final[:, :, i], mask=nans_here[:, :, i]), m.meta)
